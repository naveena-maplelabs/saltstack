...
def __virtual__():
    ...
    #   return (False, "The cohesity  module is not implemented yet")
...
def create_vmware_protection_job(
    ...
    try:
        ...
        resp = cohesity_client.protection_jobs.get_protection_jobs(names=job_name, is_deleted=False)
        ...
        if body.parent_source_id == -1:
            ...
            if pause_job:
                ...
                cohesity_client.protection_jobs.change_protection_job_state(resp.id, jobstate_body)
            return "Successfully created ProtectionGroup: {}".format(body.name)
    except APIException as err:
        return "Error creating job {} {}".format(job_name, err)


def update_vmware_protection_job(job_name, vcenter_name, sources, replace_existing=True):
    """
    ...
    try:
        cohesity_client = _get_client()
        resp = cohesity_client.protection_jobs.get_protection_jobs(is_deleted=False, names=job_name)
        if not resp:
...
def update_vmware_protection_job_state(job_name, state):
    ...
    try:
        cohesity_client = _get_client()
        jobs = cohesity_client.protection_jobs.get_protection_jobs(is_deleted=False, names=job_name)
        if not jobs:
            return "Job with name {} not available.".format(job_name)
        for job in jobs:
...
def cancel_vmware_protection_job(job_name):
    ...
    try:
        ...
        return "Error while attempting to cancel the job {}, error : {}".format(job_name, err)
...
def run_vmware_protection_job(job_name):
    ...
    try:
        ...
        return "Error while attempting to start the job {}, error : {}".format(job_name, err)
...
def delete_vmware_protection_job(job_name, delete_snapshots=True):
    ...
    try:
        ...
        return "Error while attempting to delete the job {}, error : {}".format(job_name, err)
    ...
    try:
        ...
        for node in nodes:
            if node.get("nodes", []):
                ...
                if node["protectionSource"]["vmWareProtectionSource"]["type"] == source_type:
...
def restore_vms(
    ...
    try:
        ...
        body.name = task_name
        parent_id, source_id = _get_vmware_source_ids(vcenter_name, vm_names.split(","))
        ...
        resource_pool_id = _fetch_source_objects(source_objects, "kResourcePool", resource_pool)
        datastore_id = _fetch_source_objects(source_objects, "kDatastore", datastore_name)
